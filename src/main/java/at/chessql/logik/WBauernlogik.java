...
public class WBauernlogik {
    List<String> fields = new ArrayList<>();
    public WBauernlogik(String pos, Feld f) {
    ...
    public List<String> reachableFields(String pos, HashMap sit) {
        ...
        for(String s:schlagFields){
            try{
                if(sit.get(s).toString().substring(1, 2).equals("s")) {
        ...
        for(int i=0;i<reachableFields.size();i++){
            try {
                if (sit.get(reachableFields.get(i)).toString().substring(1, 2).equals("w")||sit.get(reachableFields.get(i)).toString().substring(1, 2).equals("s")) {
                    reachableFields.subList(i, reachableFields.size()).clear();
                }
                else if (sit.get(reachableFields.get(i)).toString().substring(1, 2).equals("w")==false||sit.get(reachableFields.get(i)).toString().substring(1, 2).equals("w"))
                {
